...
-- ==================== BRAND TABLE ====================
-- =========================================================================================================
-- BUT NOT REQUIRED NOW, SINCE 'nulls' already updated in staging layer with 'Unknown
  -- Variables for "Unknown" values
...
-- =======================
  -- Merge new or changed postal codes (SCD-2)
  MERGE [gold].[dim_brand] AS tgt
  USING cte_src AS src
  ON tgt.brand_skey = src.brand_skey
    AND tgt.is_current = 1
  WHEN MATCHED AND (
        tgt.brand_tier_skey <> src.brand_tier_skey
      OR tgt.brand_name_skey <> src.brand_name_skey
      OR tgt.brand_country_skey <> src.brand_country_skey
  ) THEN
  ...
  WHEN NOT MATCHED BY SOURCE AND tgt.is_current = 1 THEN
      -- Expire rows missing in source
      UPDATE SET tgt.is_current = 0;
  ;

  -- Step 2: Insert new versions for the "expired" ones
  ...
  SELECT
    ...
    1
  FROM cte_src src
  JOIN [gold].[dim_brand] tgt
    ON src.brand_skey = tgt.brand_skey
  WHERE tgt.is_current = 0
    AND NOT EXISTS (
          SELECT 1
          FROM [gold].[dim_brand] d
          WHERE d.brand_skey = src.brand_skey
            AND d.is_current = 1
        );
